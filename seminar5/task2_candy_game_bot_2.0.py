...
def get_rules(players):
    ...
    first = int(input(
        f'{players[0]}, если хотите ходить первым, нажмите 7, если нет, любую другую клавишу\n'))
    if first != 7:
        first = 0
    ...
    while rules[0] > 0:
        if not count % 2:
            ...
            if move > rules[0] or move > rules[1]:
                print(
                    f'А вы не слишком много себе позволяете, можно взять не более {rules[1]} конфет{letter}, у нас всего {rules[0]} конфет{letter}')
                attempt = 3
                while attempt > 0:
...
if not winer:
    ...
    print(
        f'Congratulations! Сейчас победил {winer}! Теперь у него будет диатез! Пожелаем ему удачи.')